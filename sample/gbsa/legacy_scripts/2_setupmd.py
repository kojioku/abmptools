...
"""
        ...
        # leaprc ファイルの内容 (ligand)
        leaprc_ligand_content = f"""
# gengeral
verbosity 2
source leaprc.protein.ff14SB
source leaprc.DNA.OL15
...
"""
        ...
        # leaprc ファイルの内容 (receptor)
        leaprc_receptor_content = f"""
# gengeral
verbosity 2
source leaprc.protein.ff14SB
source leaprc.DNA.OL15
...
"""
        ...
        with open("leaprc_ligand", "w") as file:
            file.write(leaprc_ligand_content)
        with open("leaprc_receptor", "w") as file:
            file.write(leaprc_receptor_content)

        # tleap を実行する (complex)
        subprocess.run("tleap -f leaprc_complex > leap_complex.log 2>&1", shell=True, check=True)
        # tleap を実行する (ligand)
        subprocess.run("tleap -f leaprc_ligand > leap_ligand.log 2>&1", shell=True, check=True)
        # tleap を実行する (receptor)
        subprocess.run("tleap -f leaprc_receptor > leap_receptor.log 2>&1", shell=True, check=True)
