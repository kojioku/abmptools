...
| `<prefix>_action.bdf` + `<prefix>_show.act` | Mol_Name 維持 + Python action (`colorize_mode` が `action` / `both` の時)。**官能基単位の色付け** |
...
### 6. Python action 経由の per-functional-group 色付け (`<prefix>_action.bdf` + `<prefix>_show.act`)
...
| `<prefix>_show.act` | 各 carboxyl / amide の atom indices + role を埋め込んだ `autorun: showHbond()` action |

仕組み:

- UDF を開くと gourmet が `Action:` 行を読み、`<prefix>_show.act` を自動 load
- `autorun` は file open 時 + record 切替時に毎回実行され、`sphere()` で
  各官能基の atom (carboxyl: c/o/oh/ho、amide: c/o/n) に role 色を上書き描画
