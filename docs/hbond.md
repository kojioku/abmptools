...
「官能基単位の役割比率」に統計指標を切り替え。OCTA viewer プリ描画用に Mol_Name 維持
...
| `<prefix>.bdf` | 入力 BDF を Mol_Name 維持でコピー + 各 functional-group atom の `Attributes[]` に `Name='hbond' Value='Dual'/'Single'/'Free'/'Accept'` を append (**OCTA viewer プリ描画用**、Attribute フィルタで官能基カテゴリ可視化可) |
...
### 5. OCTA viewer プリ描画用コピー (`<prefix>.bdf`) + Attributes[] タグ

OCTA viewer の「プリ描画」 (起動時の自動描画) は Mol_Name の値を内部で参照しており、
`molecular` 等の元名前を別の文字列にリネームすると分子が空表示になる。
そのため、Mol_Name 維持のコピーを `<prefix>.bdf` として併出する:

| ファイル | Mol_Name | 用途 |
|---|---|---|
| `<prefix>.bdf` | `molecular` 維持 | **OCTA viewer プリ描画** (色付けなし、起動時自動描画される) |
| `<prefix>_colored.bdf` | `IMC_DUAL/SINGLE/FREE` リネーム | **OCTA ポスト描画** (gourmet または OCTA viewer で show アクション実行時に 3 色塗分け) |
...
既存の `Attributes[]` (OCTA viewer が書く `Name='1' Value='molecular:<id>'` 等) は
**触らず末尾に push** するので round-trip 安全。OCTA viewer の Attribute フィルタで
...
| ファイル | 内容 |
|---|---|
| `<prefix>_action.bdf` | 入力 BDF をコピーし、ヘッダの `Action:` フィールドに `<prefix>_show.act` を追記。**Mol_Name は変更しない** (OCTA viewer プリ描画でも分子形状が描画される) |
| `<prefix>_show.act` | `autorun: showHbond()` 形式の action ファイル (各 carboxyl / amide の atom indices + role を埋込)。**OCTA gourmet 用** |
| `<prefix>_show.py` | 同じ描画ロジックを autorun ラッパーなしの **プレーン Python script** として出力。**OCTA viewer (GOURMET) の Python パネル用** (autorun 形式で落ちるケースに対応) |
...
- **OCTA viewer (GOURMET) 経路** (autorun が落ちる場合): `<prefix>.bdf` (Mol_Name 維持
...
**メリット**:
- Mol_Name 維持 → OCTA viewer プリ描画と互換
...
--classify-mode {imc,generic,auto}
...
--colorize-mode {molname,action,both}
                          ...
                          action: Python action 経由 (Mol_Name 維持、OCTA viewer 互換);
                          both: 両方併出
...
print(f"uncolored bdf (OCTA viewer pre-render): {paths['uncolored']}")
```
...
### gourmet (Linux) / OCTA viewer ポスト描画
...
`output/imc_hbond_colored.bdf` (Mol_Name リネーム済) は OCTA viewer のプリ描画 (起動時の
...
| `<prefix>_show.py` | Python パネル用 plain script (OCTA viewer 用) |
...
### OCTA viewer Attribute フィルタとの組合せ

`<prefix>.bdf` を OCTA viewer で開いて Attribute フィルタで `hbond=Donor` / `Acceptor` /
