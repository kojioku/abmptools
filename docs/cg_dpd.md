# abmptools.cg.dpd — CG → DPD 系入力 (Cognac UDF / OCTA viewer dpm) ビルダー
   ...
   monomer + calc_sett                  aij.dat (Python 辞書)
        ...
        +-----------+----------------------+
            ...
            *_uin.udf   *.dpm + monomer-lib/<seg>/Virtual.mom + #Message.txt
            (Cognac      (OCTA viewer GUI 用、 編集後 UDF 出力)
             直接実行)
```
...
| **R1** (新) | `{name}_monomer` + `{name}_calc_sett` + `aij.dat` | `*_uin.udf` (Cognac DPD 入力) | **UDFManager (named-path) writer、cognac112 スキーマ準拠 = cognac ロード可能。OCTA 必須** |
...
| **assign-aij** (新) | 既存 Cognac DPD UDF + `aij.dat` | UDF の `Interactions.Pair_Interaction[].DPD.a` を上書き | 粒子名で順不同照合、UDFManager round-trip |

> **R1 writer の変更 (重要)**: 旧 plain-text writer は positional で Pair_Interaction を
> 書いており **cognac112 スキーマ非準拠 = UDFManager ロード不可** だった。現在は
> `udf_writer_udfm.py` が UDFManager の named-path put で組み立て、`Pair_Interaction` を
> 正しく `Interactions[]` 配下に置き `Molecular_Attributes.Interaction_Site_Type[]` を
> 定義する。**`build-udf` は UDFManager (OCTA) 必須**になった (旧「UDFManager 非依存」
> 設計は廃止)。`tests/cg_dpd/test_udf_writer.py` は実ロード検証 (loadability) に刷新。

## 権利配慮の設計

OCTA viewer は商用ソフトであり、 dpm の class 定義 (`\begin{def}` 800+ 行) は OCTA 公開
仕様には含まれない可能性があるため、 abmptools は以下の方針:

| File 種別 | abmptools の出力範囲 | 解決方法 |
|---|---|---|
| **Cognac UDF** (R1) | 冒頭 `\include{"cognac112.udf"}` 1 行で class 定義を委譲。値は UDFManager の named-path put で設定 | OCTA viewer install dir 経由で UDFManager が class 定義を resolve (= 生成にも OCTA 必須) |
| **DPM** (R2) | `\begin{data}` 内の 5 top-level block のみを **patch** (template の def section は触らない) | **B 案** = user が OCTA viewer で空 dpm を 1 回作成して abmptools に渡す |
...
# R2: OCTA viewer dpm + monomer-lib (要 user 提供 template)
dpm = builder.build_dpm(
    template="empty.dpm",                  # user が OCTA viewer で 1 回作成
    output_dir="./chol_project",
    virtual_mom_template="Virtual.mom",    # user が OCTA viewer Monomer Modeler で 1 回作成
...
# assign-aij: 既存 DPD UDF の Pair_Interaction.DPD.a を aij.dat で上書き
#   粒子名 (Site1_Name/Site2_Name) で順不同照合。chi モードは a=aii+χ/0.306 に変換
python -m abmptools.cg.dpd assign-aij \
    --udf existing_uin.udf --aij aij.dat --aii 25.0 \
    --output existing_patched.udf
```
...
template の class 定義 (OCTA viewer 商用 spec) は **触らずに温存**。
...
- **R2**: dpm template path + Virtual.mom path + 出力 dir + dpm filename + `[Build R2 DPM]` ボタン (template path 必須、 OCTA viewer で作成)
