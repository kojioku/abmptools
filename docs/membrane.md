...
### pull geometry
`direction` を採用。`distance` は常に正の値しか返さないので両側 PMF
(z_min < 0 < z_max) で破綻。`direction-periodic` は semiisotropic Pcoupl で
GROMACS が拒否。`direction` は signed coord を返し、peptide が box 内に
留まる限り well-defined。
