...
```
output_dir/
├── {name}_monomer       # bond12 / bond12h / bond13_180 / bond13_120 (Python script)
...
1. **共有 atom がある** (fused ring boundary) — `seg_i.atom_indices ∩ seg_j.atom_indices != ∅`
2. **直接 atom 結合がある** (cap 経由 / ring-chain 境界) — `mol.GetBondBetweenAtoms(...)`
3. **Fused ring group の全結合化** (剛直性維持): step 1/2 で連結された ring 群を
   union-find でグループ化し、同じ group 内のすべての ring pair に bond12 を追加
   (cholesterol の A-B-C-D fused ring なら A-C / B-D / A-D も bond12 に追加)
...
### bond13_180 / bond13_120 自動推定

| 種類 | 検出ロジック | distance | stiffness |
|---|---|---|---|
| **bond13_180** (1-3 直線) | bond12 graph で **path length 2** の (i, j, k) ペア (bond12 既存ペアは除外) | 1.72 | ring 両端: 200 / chain or mixed: 80 |
| **bond13_120** (1-3 シス二重結合) | RDKit `BondType.DOUBLE` の両端 atom の **隣接 atom** を辿って 3 segment 間の 1-3 を検出 | 1.49 | 70 |

- distance 1.72 = 2 × 0.86 (直線配置時の 1-3 距離)
- distance 1.49 = シス二重結合の 1-3 距離 (cholesterol sample 流儀)
- 化学的に「真の直線」「真のシス」かは保証されない。geometry を見て、必要なら
  monomer file を手動編集 (該当 entry を削除 / 別 type に移動) すること。
...
29 テスト (dataclass roundtrip / e2e 7 / 編集 op 7 / DPDgen 11 + CLI 系)、~0.5s で PASS。
