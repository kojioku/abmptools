...
> あります。 Windows native ユーザーへの配布についても同 doc を参照。
...
Reading is done entirely in Python. A Fortran shared library used to provide an
alternative reader; it was removed once the Python reader became the faster of
the two, and it never supported `GRIMME-MP4`, PB-IFIE, BSSE-IFIE or the
monomer/dimer energies.

`MP2` / `MP3` / `CCPT` (MP4) logs are all read by the same path; the column set
follows the log's `Method` keyword.
