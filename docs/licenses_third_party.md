...
**License 戦略**: insane (GPL-2.0) を取り込まず subprocess で呼ぶことで、abmptools 本体の license (現 MIT、将来 Apache-2.0 計画あり) を維持。詳細は `docs/cg_membrane.md` の "License rules" 節参照。
...
**License 戦略**: ase (LGPL-2.1+) は import 利用のみ (subprocess ではなく Python API 呼び出し)。LGPL の "linking exception" / "library use" 条項により、abmptools 本体 (MIT、将来 Apache-2.0 化計画あり) を ase に感染させることはない (改変・再配布する ase バイナリ・ソースを同梱しない限り問題なし)。ABINIT-MP は subprocess only で外部バイナリ呼び出しのため、abmptools の license に影響なし。詳細は `docs/crystal.md` の Scope 節参照。
