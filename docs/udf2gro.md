...
#### `tau_p` を変えるとどれだけ遅くなるか

**箱の応答は `tau_p` に比例する。** 水 2165 分子で `tau_p` を振り、体積の
自己相関の 1/e 減衰を測った実測 (2026-09-12):

| `tau_p` | 減衰時間 | 減衰 / `tau_p` |
|---|---|---|
| 4 ps | 0.785 ps | 0.196 |
| 8 ps | 1.562 ps | 0.195 |
| 16 ps | 3.026 ps | 0.189 |

係数は 0.19 で安定している。**収束の目安は減衰の 3〜5 回分**なので:

```
tau_p =  2 ps   →  収束まで  1〜2 ps
tau_p =  5 ps   →           3〜5 ps
tau_p =  8 ps   →           5〜8 ps
tau_p = 20 ps   →          12〜19 ps
```

`tau_p` を 2 → 8 にすると箱の応答は 4 倍遅くなるが、**絶対値はどれも
10 ps 台**である。

#### 律速がどちらかで決まる

**バロスタットと材料自身の構造緩和の、遅い方が効く。**

| 系 | 材料側の緩和 | 判定 |
|---|---|---|
| ポリマー融体 (Tg 近傍) | ns 〜 μs | **バロスタットは律速でない。`tau_p` 8 でも 2 でも密度の追従は変わらない** |
| 低分子液体・水 | ps 台 | バロスタットが律速になりうる。`tau_p` を小さく |

効いてくるのは次の場合。

- **各段階の保持が 50 ps 未満**。`tau_p` = 8 ps だと 6 周期しか回らない
- **急な温度ジャンプの直後**。追従に 5〜8 ps かかるので、刻みが細かいと積み残す
- **降温ランプで ρ(T) を読む** (Tg 等)。箱が追従できているか、ρ(T) に遅れや
  階段状の癖が出ていないかを見る

逆に、`tau_p` を小さくしすぎると箱が振動しやすくなる。**遅い方が安全側**なので、
律速でないなら無理に下げなくてよい。
