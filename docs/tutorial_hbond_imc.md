...
| **J-OCTA Viewer** | `<prefix>.bdf` を開いて Python パネルで `<prefix>_show.py` を Load → Run、または Attribute フィルタで `hbond=Dual/Chain/Single/Free/Accept` 絞り込み |
...
### 4.2 J-OCTA Viewer — Python panel (autorun crash 回避)

J-OCTA Viewer は autorun action 形式で crash することがある。その場合:
...
### 4.3 J-OCTA Attribute フィルタ — 色なしカテゴリ可視化

`output/imc_hbond.bdf` の各 functional-group atom の `Attributes[]` に
`hbond=Dual/Chain/Single/Free/Accept` が append されているので、J-OCTA の
Attribute フィルタで:
...
を実行する必要がある (J-OCTA プリ描画では空表示になるので注意)。
...
### 7.4 J-OCTA Viewer で `<prefix>_action.bdf` を開いた瞬間 crash

J-OCTA は autorun action 形式で落ちることがある。`<prefix>.bdf` (Mol_Name
維持コピー) を開いてから Python panel で `<prefix>_show.py` を Load → Run
する経路 (4.2) を使う。

### 7.5 J-OCTA Viewer プリ描画で分子が空表示

`<prefix>_colored.bdf` は Mol_Name が `IMC_DUAL/CHAIN/SINGLE/FREE` にリネーム
されており、J-OCTA 内部の Mol_Name lookup が壊れる。`<prefix>.bdf` (Mol_Name
維持) を使う。
