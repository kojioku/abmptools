...
centerinfo=":1"
tgt1=':WAT@O'
tgt2=':WAT' #WAT #KET #PVP
option='byres2 nointramol'
# --- user setting end ---
...
# --- parm print section ---
echo stime: $stime, etime: $etime, sampletime: $sampletime
echo centerinfo, $centerinfo

echo sframe, $sframe, eframe, $eframe
sleep 3
...
if [ $stime_inprod -eq 0 ]; then
    ...
    # intxt="`ls $trajs_filt` ucell"
    # echo $intxt >> inbuf.txt
...
do
    # echo $line
    if [ $line = "init.coor" ]; then
        echo "trajin $line" >> cpptraj.in
    else
        echo "trajin $line ucell" >> cpptraj.in
    fi
done < inbuf.txt
...
echo "radial ${dir}/${ofile} 1 20 $tgt1 $tgt2 volume $option" >> cpptraj.in
echo 'run' >> cpptraj.in
cpptraj < cpptraj.in

cp $prmtop $dir
echo "${dir}/${ofile} ($stime,$etime) was generated."
