...
DEFAULT_COLORS: Dict[str, DrawAttribute] = {
    ...
    "chain":  DrawAttribute("_CHAIN",  "Magenta", transparency=1.0, radius=1.0),
...
def colorize_udf(
    ...
    # Draw_Attributes.Molecule[] -- add one entry per known role
    u.jump(-1)
    existing = u.size("Draw_Attributes.Molecule[]") or 0
    for idx, role in enumerate(("dual", "chain", "single", "free")):
        if role not in color_attrs:
            continue
...
DEFAULT_ACTION_COLORS: Dict[str, List[float]] = {
    "dual":   [1.0, 0.0, 0.0, 1.0],     # red    (cyclic COOH dimer)
    "chain":  [0.85, 0.0, 0.85, 1.0],   # magenta (one-way COOH-COOH chain)
    "single": [0.0, 0.0, 1.0, 1.0],     # blue   (COOH -> amide donor)
...
def _render_show_action(
    ...
    lines = [
        ...
        "# Carboxyl: dual=red, chain=magenta, single=blue, free=gray.",
...
def write_hbond_attributes(
    ...
    if value_map is None:
        value_map = {
            "dual": "Dual", "chain": "Chain", "single": "Single",
