...
class molcalc():
    ...
    def Exportardpos(self, path: str, iname: str, molindex: list[int], posMol: list[list[list[float]]], nameAtom: list[list[str]], molnames: list[str] | None = None) -> None:
        """指定した分子インデックスの座標をXYZ/PDBファイルに出力する。

        PDB は obabel 経由 (XYZ→PDB) でなく Python の直接 writer で生成する。
        obabel は XYZ から bond perception を行うため、 PBC 境界を跨いだ broken
        molecule (例: TIP3P water で O と H が boundary 反対側に分かれた状態) で
        H atom が orphan として `ATOM/UNK` 行に分類されてしまう。 直接 PDB writer
        は mol 単位の atom 構成 (`nameAtom[i]`) と mol 名を信頼するため、 broken
        state でも全 atom が正しい residue に分類される。
        ...
        # PDB を直接生成 (obabel bypass)
        pdb_path = out_head + ".pdb"
        self._write_pdb_direct(pdb_path, molindex, posMol, nameAtom, molnames)

    @staticmethod
    def _atom_name_to_element(atom_name: str) -> str:
        """`Atom_Name` (e.g. 'O0', 'H17', 'Cl3') から PDB element symbol を抽出。

        英字 1〜2 文字 (先頭が大文字、 2 文字目があれば小文字 or 大文字) を返す。
        ベンチマーク: 'O0'→'O', 'C13'→'C', 'Cl3'→'CL' (大文字), 'H37'→'H'.
        """
        if not atom_name:
            return 'X'
        if len(atom_name) >= 2 and atom_name[0].isalpha() and atom_name[1].isalpha():
            return atom_name[:2].upper()
        return atom_name[0].upper()

    @staticmethod
    def _molname_to_resname(molname: str | None) -> str:
        """`Mol_Name` (e.g. 'A_PEG5', 'B_water') から PDB residue name (3 char) を派生。

        Returns: 大文字 3 文字。
        'A_PEG5' → 'PEG', 'B_water' → 'WAT', 'methanol' → 'MET', etc.
        """
        if not molname:
            return 'UNK'
        # Strip leading 'A_' / 'B_' / 'mol_' prefix when present
        s = str(molname)
        for prefix in ('A_', 'B_', 'C_', 'D_', 'mol_'):
            if s.startswith(prefix):
                s = s[len(prefix):]
                break
        # Special-cases
        lo = s.lower()
        if lo in ('water', 'h2o', 'hoh'):
            return 'HOH'
        return (s[:3] or 'UNK').upper()

    def _write_pdb_direct(self, pdb_path: str, molindex: list[int],
                          posMol: list[list[list[float]]],
                          nameAtom: list[list[str]],
                          molnames: list[str] | None) -> None:
        """obabel を介さずに PDB を書き出す。 1 mol = 1 residue で連番化。"""
        serial = 0
        with open(pdb_path, "w") as f:
            f.write("COMPND    Generated by abmptools Exportardpos\n")
            f.write("AUTHOR    abmptools._write_pdb_direct\n")
            for out_idx, i in enumerate(molindex):
                # PDB residue id は 4-digit、 9999 で wrap
                resid = (out_idx + 1) % 10000
                molname = molnames[out_idx] if (molnames and out_idx < len(molnames)) else None
                resname = self._molname_to_resname(molname)
                for j in range(len(posMol[i])):
                    serial += 1
                    sn = serial if serial < 100000 else serial % 100000
                    atom_name = str(nameAtom[i][j])[:4]
                    elem = self._atom_name_to_element(atom_name)
                    x, y, z = posMol[i][j][0], posMol[i][j][1], posMol[i][j][2]
                    # PDB strict column format:
                    # 1-6 record, 7-11 serial, 13-16 atom_name, 17 alt, 18-20 resname,
                    # 22 chain, 23-26 resid, 31-38 x, 39-46 y, 47-54 z,
                    # 55-60 occ, 61-66 temp, 77-78 element
                    # Standard PDB strict column layout:
                    #   1-6 record, 7-11 serial, 12 space, 13-16 atom name,
                    #   17 alt loc, 18-20 resname, 21 space, 22 chain,
                    #   23-26 resid, 27 iCode, 28-30 spaces, 31-38 x,
                    #   39-46 y, 47-54 z, 55-60 occ, 61-66 temp,
                    #   67-76 spaces, 77-78 element
                    f.write(
                        "HETATM"
                        f"{sn:>5d}"
                        " "
