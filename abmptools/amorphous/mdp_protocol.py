...
def write_udf_export_script(output_dir: str,
    ...
    Generates a cross-platform Python script that dumps two OCTA
    ...
    * ``<stage>_nojump.gro`` — :func:`abmptools.trajectory.nojump`
      (``gmx trjconv -pbc nojump``)。 OCTA viewer (GOURMET) + gro2udf 下流
      用に分子を連続化。
    ...
    lines = [
        "#!/usr/bin/env python",
        '"""Post-processing: export UDF-compatible inputs from MD outputs.',
        "",
        ...
        "                              viewer (GOURMET) and downstream UDF conversion",
