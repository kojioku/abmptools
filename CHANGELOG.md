...
`-m post` をかけると、`read_ifie` が存在しない `.out` を開いて IOError を握り潰し、
**ielist が全部 0 のまま RC 0 で完走**した。`agg_result` は tar の有無で
`eoutflag` を立てるので、`-m arrange` まで正常に見える (fcews の e2e 再現で発覚)。

最初の修正 (tar を展開し、読み終わったら `.out` を消す) には**データを壊す経路が
2 つ**あったので、公開前に作り直した:

- `tar.extractall()` は既存の `.out` を上書きする。**pack 後に計算し直した新しい
  版が tar の古い版に置き換わり、古い版で集計される**
- 後片付けの `del_out` はディレクトリの `*.out` を全部消す。**tar に入っていない
  `.out` も消え、戻せない**

いずれもエラーを出さない。

**展開も削除もやめ、ディスクにあればディスク、無ければ tar の member を直接読む。**
何も書かず何も消さない。4 つの読み取り関数 (`read_ifie` / `read_ifiepb` /
`captfmomp2e` / `getfmopbenergy`) を `_read_out_lines` 1 つに通した。**BE 経路
(`getTE`) の `unpack_tar` 呼び出しも撤去**した —— こちらは削除はしないが上書きは
しており、**2.16.0 以前から**あった挙動。

- tar はディレクトリごとに 1 回だけ開いて索引を作る (1 本ごとに開き直すと 5000 本で
  O(N²))
- `unpack_tar` は明示利用のために残し、**ディスクに無い member だけ**取り出す形に
- 旧テストの fixture は `getifiesum` の照合に当たらず、**全部 0 の ielist 同士を
  比べて一致**していた。値の違う tar とディスクを置き、どちらが届いたかを見る形に

テストは旧実装で 6 件が落ち、新実装で通ることを確認済み。
...
### Fixed — `gro2udf --help` にオプション一覧が出ていなかった

2 行の usage を出して**「全部見るなら `--from-top --help`」と案内**していた。
**ほとんどの人がここに来る理由である `--trajectory` が、実際に読まれる
help に載っていない** (2026-09-16、利用者の指摘)。

`--help` が**本物のオプション一覧**を出すようにした。`--trajectory` が
`.xtc` を受けることも、`--energy` も、そこに書いてある。**案内の中に案内を
置かない。**
...
`convert_gromacs_udf.py` は、その列を**コメントアウトして読んでいない**
(11.1 / 12.0 とも)。右から数える実装なので 6 列でも 7 列でも動くが、
**原子番号の値は捨てられる**。

**元素を決めている唯一の経路は `[ atoms ]` の質量列**だった (238-246 行)。

```python
if len(stmp) > 7:                   # [ atoms ] が 8 列以上のときだけ
    mass = float(stmp[7])
    if round(mass) in mass_map:     # conf/mass.conf の 質量 -> 元素 表
        atom = mass_map[round(mass)]
```

書いていた `[ atoms ]` は 7 列で、この分岐に入らない。だから原子名は
力場の型名 (`hc1`) のまま残り、**全原子系が粗視化として読まれていた**。

**質量列は GROMACS では省略可** —— 無ければ `[ atomtypes ]` から引く、
というのが規約で、その規約を実装していない読み手には明示が要る。
実機で両方向を確認した (足すと `Atom_Name` が `['H','C','C','H',...]`
になり、足さないと `['hc1','c30',...]` のまま)。

### Fixed — `udf2gro` の `[ atomtypes ]` に原子番号が無かった

**J-OCTA の CG 誤認を追う過程で見つけた**が、**これは原因ではなかった**
(上の `[ atoms ]` の質量列が本命)。原子番号が無いこと自体は、
他の読み手にとっては情報の欠落なので直しておく。

`[ atomtypes ]` を **`name mass q ptype sigma eps` の 6 列**で書いていて、
**原子番号の列が無い**。列にある名前は力場の型名 (`c3` / `os` / `hc`) で、
**その力場の外では元素を意味しない**。`[ atoms ]` の原子名もそこから作る
ので、**top の中に元素を判定する材料が質量しか残っていなかった**。

```
;  atom type  at.num         mass            q particle_type ...
          c3       6        12.01          0.0            A  ...
          os       8         16.0          0.0            A  ...
```

**GROMACS は原子番号を必要としない**ので、この形でも動いてしまい、
**読み込む側で初めて分かる**。質量から引いて書くようにした。

> **粗視化ビーズには元素が無いので、そこは 0 のまま**にする (GROMACS が
...
原子ごとの部分電荷 [e] をファイルで渡すと、**その成分では電荷バックエンドを
走らせず、渡された値をそのまま使う**。並びは構造の原子順、区切りは空白か
改行、`#` 以降はコメント。

**狙いはポリマー。** 鎖を丸ごと AM1-BCC に掛けるのは、**遅いだけでなく
当てにならない** —— ランダムウォークで組んだ鎖は自己重なりを持つので、
電荷そのものが壊れる。繰り返し構造を知っている呼び出し側が、**短い
オリゴマーで求めてユニット単位に展開**し、その結果をここへ渡せる。

```bash
python -m abmptools.amorphous --mol pva_10mer.sdf --charges pva10.q ...
```

**どう展開するかは化学に依るので、呼び出し側に置いた。** この層が約束するのは
「渡された数字をそのまま使う」ことだけ。

原子数が合わなければ `ValueError` で止める —— **黙って切り詰めたり循環させたり
すると、走りはするが答えが違う系ができる**。総電荷が整数から離れているときは
警告する (たいてい別の分子のファイルを渡している)。
...
`tau_p` を振り、体積の自己相関の 1/e 減衰を測った (係数 0.19 で安定)。

```
tau_p =  2 ps   →  収束まで  1〜2 ps
tau_p =  5 ps   →           3〜5 ps
tau_p =  8 ps   →           5〜8 ps
```

`2 → 8` で 4 倍遅くなるが、**絶対値はどれも 10 ps 台**。

**律速はバロスタットと材料自身の構造緩和の遅い方**なので、ポリマー融体
(緩和 ns〜μs) では `tau_p` が 8 でも 2 でも密度の追従は変わらない。効くのは
各段階の保持が 50 ps 未満のとき、急な温度ジャンプの直後、降温ランプで ρ(T) を
読むとき。低粘度の液体ではバロスタットが律速になりうる。

`AnnealProtocol.tau_p` のコメントからも参照できるようにした。
...
> 含む熱浴質量なので、`tau_t` に直すには `g·k_B·T` で割る (`docs/udf2gro.md`)。
> `.mdp` は雛形と考え、本番では見直すこと。
...
- `#### 6.3.1 入れる — `--user --no-deps` (既定)` /
  `#### 6.3.2 別環境に作る場合 — venv + constraints (サブルート)` に組み替えた
- **`B-0` / `B-1` / `B-2` という呼び名をやめた。** 目次と見出しがどちらが既定かを
  伝えないので、読み手が毎回本文に戻ることになる。本文中の参照も節番号に置換した
- 比較表を「使うまでの手間」から始まる形に直した。依存解決の有無より、**毎回の
  手間があるかどうか**の方が先に効く
- moldeck の配布 zip には同じことをする `.bat` (`install_minimum.bat` /
  `install_for_hbond.bat`) が入っている旨を追記した。abmptools は PyPI 配布なので
  スクリプトを同梱できない

moldeck 側の `docs/INSTALL.md` と同じ構成にしてある。両方を読む利用者が突き合わせ
られるようにするため。
...
`udf-and-gro モード` と `--from-top モード` が、ディレクトリ一覧の見出しとして
いきなり出てくるだけだった。読み手はソースのフォルダ名からしか意味を推測できない。
**「どちらを使うか」に先に答える節**を冒頭に置いた。分かれ目は
**元になる UDF があるかどうか**で、udf-and-gro は既存 UDF の座標を差し替え、
`--from-top` は top から UDF を新規に作る。

IMC + PVP の 30 分子系で実測: udf-and-gro は `Set_of_Molecules` ブロックが
726,489 文字バイト一致のまま座標だけ入れ替わる。

### Docs — amorphous のチュートリアルから UDF 変換へ繋いだ

非晶質セルを組んで MD を回した後にやることが変換なのに、手順を追う
`amorphous_tutorial.md` は MD で終わっていて変換に触れていなかった。逆に
`gro2udf.md` は入力の出所を書いていなかった。両方向に導線を張った。

**書いていて既存手順の誤りが 1 件見つかった。** `amorphous.md` の標準フローが
`--trajectory` / `--energy` に渡しているファイルは **MD を回しただけでは
生成されない** (`md/gen_for_udf.py` が作る)。チュートリアル側は最小構成を先に
示し、全部入りはその入力を作る手順の後に置いた。実測で 5.0 MB (1 record) /
12.7 MB (101 record)。
...
通し番号で、分子種ごとに 0 に戻る。`_display_type_name` はこれを `<元素><index>` に
書き換える (OCTA viewer が先頭文字で元素を判定するため) が、その書き換えで
**分子種を区別する情報が落ちる**。結果、元素と位置が一致する型どうしが衝突する。

実測 (アリピプラゾール + 乳酸): `APZ_2` (O, σ=0.30251 ε=0.70486) と
`LAC_2` (O, σ=0.30398 ε=0.87950) が**どちらも `O2`**。UDF は同名の `Atom_Type` を
2 つ持ち、名前で型を引く経路 (`interaction_Site` / `Pair_Interaction` /
読み戻す全コンバータ) は先頭を拾うので、**乳酸の O がアリピプラゾールの O の
パラメータで走っていた**。

- 型 69 個に対しユニークな名前 68 種
- 往復させた topology の **LJ-14 が 2568.80 → 2545.70 kJ/mol (0.9 %) ずれる**。
  Coulomb-14 は一致するので、対の一覧ではなくパラメータの取り違えと分かる
- **単一成分では起きない**。多成分系だけが静かに壊れる

topology 全体で一度に名前を割り当てる `build_display_type_map()` を追加し、
3 箇所の呼び出し (`Set_of_Molecules` / `Molecular_Attributes` / `Interactions`)
を同じマップに統一した。割り当ては

1. 先に現れた型が `<元素><index>` を取る → **単一成分の出力は従来とバイト一致**
   (リポジトリ同梱の単一成分 fixture で確認)
2. 衝突した型は、その元素で既に使われている最大 index の次を取る
   (最小の空き番号ではなく最大+1。元の範囲外だと「振り直された」と分かるため)
3. 力場固有の型名 (GAFF `c3` 等) は素通しで、生成名より先に予約する

**黙って改名しない**: 衝突を解消したときは警告を出す。改名後の index は
interchange の index と一致しないので、知らせないこと自体が次の罠になる。

検証: 修正後は 69 型 → 69 名でユニーク、往復後の LJ-14 が 2568.7983 と
元の topology (2568.7981) に一致。Bond / Angle は厳密一致。

**2.13.6 以前が書いた多成分 UDF は作り直すこと。** 名前が潰れている以上、
どちらの型のパラメータが残っているか UDF だけからは判別できない。

## [2.13.6] - 2026-08-30

### Fixed — udf2gro: 多重度項が黙って捨てられ、二面角が 1 項に潰れていた

GAFF / AMBER の二面角は同じ 4 原子に対する複数のフーリエ項の和で、GROMACS では
funct 9 (proper dihedral, multiple) の行を項の数だけ並べて表す。
`_dedup_dihedrals` は**4 原子組だけをキーに重複判定していた**ため、
**2 項目以降がすべて「重複」として捨てられ**、その二面角は最初に来た 1 項に
潰れていた。

- 実測 (アリピプラゾール + 乳酸): **268 項中 39 項が消失**。往復させた topology は
  229 組中 35 組で元の `.top` と食い違った
- 回帰コーパス 105 系では **103,993 → 128,518 項** (+24,525)。GAFF を使う系なら
  ほぼ必ず踏む
- **静かに壊れる**: 行数が減るだけで `gmx grompp` は通り、警告も出ない

funct とパラメータまで含めて重複判定するようにした。合わせて、同じ 4 原子組に
複数の proper 項が残った場合は funct 1 → 9 に昇格させる (**GROMACS は funct 1 を
1 組につき 1 行しか受け付けない**。関数形は同一)。

自己二面角 (原子の重複がある組) を落とす従来の挙動と、真の重複 (4 原子・funct・
パラメータすべて一致) を畳む挙動は維持。角度・結合は影響なし
(`_dedup_angles` は同じ問題を持たない — 実測でも複数項ゼロ)。

検証: 元 `.top` が分かる系で往復させ、**二面角 268 項すべてが復元**され
パラメータまで一致 (APZ 206/206 組、LAC 23/23 組)。単点エネルギーで
Bond / Angle が厳密一致、二面角の総和も一致。回帰コーパス 105 系の差分は
**`[dihedrals]` の追加のみ**で `.gro` / `.mdp` は不変、抜き取り 6 系で `gmx grompp` 成功。
...
再開の判断が「ファイルがあるか」だけだった。

```bash
if [ -s "${d}/${d}.gro" ]; then echo "[skip] ${d} は minimize 済み"; continue; fi
```

**`mdrun` が `.gro` を書いている最中に殺されると、切り詰められたファイルが
残る。空ではないのでこの検査を通り、完成扱いで飛ばされる。** そのまま壊れた
構造で液滴ができる。`mkdir` は作業の前に走るので、ディレクトリの存在も何も
保証しない。`arrangetraj()` に至ってはガードが無く、毎回やり直したうえ
`mv -f` で前回の液滴を潰していた (cpptraj が `.1` に書いて上書きを避けたものを、
スクリプトが明示的に被せ直していた)。

- **段ごとの完了マーカー** (`${d}/.done.minimize` / `.done.arrange`) を導入し、
  判断はこれだけで行う。**マーカーは中身を検めたあと最後に書く**ので、
  途中で落ちればマーカーは無く、次回はやり直しになる
- **中身の検証**: `.gro` は 2 行目の原子数と行数 (N+3) の一致と最終行が
  箱ベクトルであること、`.pdb` は原子行の存在と `END` (または最終行が座標欄まで
  書けた原子行) を見る
- **液滴は一時名に書かせてから rename**。同一 fs 内の rename は原子的なので、
  殺されても半端な `_mask.pdb` が残らない
- マーカーが無いのに成果物がある場合は、**完全なら採用してマーカーだけ付け、
  壊れていれば park してやり直す**。マーカー導入前の実行結果もそのまま活きる
- **`--check`**: 何も実行せず、フレームごとに 済 / 途中 / 未 を表示する。
  prmtop が無い段階でも見られる。**`--redo` / `--redo-from <stage>`** で強制やり直し

### Changed — tips/md-fmo: 上書きで消さない (配布対象外)

`mv -f` を全廃し、作り直しが要る場面は `park()` で `<名前>.<日時>` にどける
(`[park]` と表示)。`-optedpdb/` へのコピーも同じ扱いにしたので、再実行で前回の
液滴が黙って消えることはない。`${head}_${i}.*` のグロブ移動も park を通す。

テスト: `tests/test_md_fmo_resume.py` (8 件)。gmx も cpptraj も使わず、
`--check` に切り詰めた `.gro` / `.pdb` やディレクトリだけの状態を食わせて、
分類を突き合わせる。
