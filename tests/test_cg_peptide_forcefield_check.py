...
class TestCheckMartiniFiles:
    def test_all_files_present(self, tmp_path):
        ...
        assert all(f.found for f in files)
        assert {f.name for f in files} == set(REQUIRED_MARTINI_FILES)
    ...
    def test_empty_dir_returns_all_missing(self):
        ...
        assert len(files) == len(REQUIRED_MARTINI_FILES)


class TestReport:
    @patch("abmptools.cg.peptide.forcefield_check.shutil.which")
    def test_buildable_when_all_ok(self, mock_which, tmp_path, capsys):
