...
-- 下流の変換器 11.1 ``python/下流の GROMACS 変換器``, lines 1722-1737

So the four-character string ``"None"`` reads as "a deformation I don't know"
and aborts the conversion.  下流の変換器 writes ``""`` in its own output, and the
...
route (OCTA8.4 / 下流の変換器) selects.

``abmptools.udf2gro`` accepts both spellings since 2.13.4, but 下流の変換器's
...
#: The guard as it appears in 下流の変換器's 下流の GROMACS 変換器.
SUPPORTED = ["Cell_Deformation"]


def downstream_guard(method):
    """Return True if 下流の変換器's exporter would accept *method*."""
...
def test_the_guard_this_protects_against():
    ...
    assert downstream_guard("None") is False           # what used to be written
    assert downstream_guard("") is True                # what is written now
    assert downstream_guard("Cell_Deformation") is True
