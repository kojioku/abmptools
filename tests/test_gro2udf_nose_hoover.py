# -*- coding: utf-8 -*-
"""Nose-Hoover の Q と Cell_Mass は UDF の規約に合わせる。

変換器はアルゴリズムごとに**別のフィールド**から `Q` を読み、
`tau_t = 2*pi*sqrt(Q*Mass*Length^2/T)` を作る。`NVT_Nose_Hoover.Q` しか
書かないと、NPT に切り替えた瞬間に 0 が読まれ **tau_t = 0** に
...
def _tau_t(q, mass=1.0, length=0.1, T=300.0):
    """g*k_B を落とした tau_t の式。"""
    return 2.0 * math.pi * math.sqrt(q * mass * length * length / T)


# 既存の UDF 4 系 (原子数, Q)。regression の実測値。
REFERENCE_Q_VALUES = [
...
@pytest.mark.parametrize("n_atoms,q_ref", REFERENCE_Q_VALUES)
def test_3n_reproduces_the_reference_q(n_atoms, q_ref):
    assert _q(n_atoms, "3N") == pytest.approx(q_ref, rel=2e-5)


@pytest.mark.parametrize("n_atoms,q_ref", REFERENCE_Q_VALUES)
def test_3n_minus_3_does_not(n_atoms, q_ref):
    """80 原子だと 3.8% ずれる。ここで規約を判別できた。"""
    if n_atoms > 1000:
        pytest.skip("大きい系では差が 0.03% で判別に使えない")
    assert _q(n_atoms, "3N-3") != pytest.approx(q_ref, rel=2e-5)
...
def test_3n_leaves_centre_of_mass_motion_alone():
    """comm-mode = None。 下流の既定と同じで、3N と整合する。"""
    m = _moment("3N")
...
def test_rotation_is_never_stopped():
    """GROMACS は回転のみの除去を受け付けない (下流が例外を投げる)。"""
    for dof in ("3N", "3N-3"):
